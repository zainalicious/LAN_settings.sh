...
enable_disable_menu() {
    ...
    # Jika user langsung tekan Enter, tidak ada perubahan
    if [ -z "$choice" ]; then
        echo "No changes made."
        return
    fi
    # Validasi input hanya 1 atau 2
    ...
    echo "After making changes, please reboot to have them take effect."
}
# Edit Simple Value
edit_simple_value() {
    local node="$1"
    ...
    read -p "Enter new $description (or press Enter to cancel): " new_value
    # Jika user langsung tekan Enter, tidak ada perubahan
    if [ -z "$new_value" ]; then
        echo "No changes made."
        return
...
edit_dhcp_range() {
    ...
    read -p "Enter new Start IP (or press Enter to cancel): " new_start_ip
    # Jika user tekan Enter di Start IP, batalkan semua
    if [ -z "$new_start_ip" ]; then
        echo "No changes made."
        return
    fi
    read -p "Enter new End IP (or press Enter to cancel): " new_end_ip
    if [ -z "$new_end_ip" ]; then
        echo "No changes made."
        return
    fi
...
main_menu() {
    while true; do
        clear
        display_red "Warning, these changes can break access over the network. Know
what you are doing, and be prepared to use ADB to fix this just in case."
